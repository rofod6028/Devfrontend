...
function UsageSummaryPage({ onBack }) {
  ...
  const [selectedIdx, setSelectedIdx] = useState(0); // 기본값 0 = 이번 달(항상 배열 첫 항목)
  ...
  const WINDOW_SIZE = 6;
  // 이번 달을 항상 첫 구간(offset 0)의 시작점으로 삼는다 (백엔드가 항상 이번 달을 인덱스 0으로 고정해서 내려줌)
  ...
  //    범위를 벗어날 수 있으므로 안전하게 0(=이번 달 또는 첫 항목)으로 보정
  const safeSelectedIdx = selectedIdx !== null && selectedIdx < entries.length ? selectedIdx : 0;
      ...
      <div className="usage-summary-print-area">
        ...
        <div className="usage-period-tabs">
          <button
            ...
            onClick={() => { setPeriod('monthly'); setSelectedIdx(0); setExpandedFacility(null); }}
          >
            월별
          </button>
          <button
            className={`usage-period-tab ${period === 'yearly' ? 'active' : ''}`}
            onClick={() => { setPeriod('yearly'); setSelectedIdx(0); setExpandedFacility(null); }}
          >
            연도별
          </button>
              ...
              {selectedFacilityUsage.map(([facility, qty]) => {
                ...
                return (
                  <div key={facility} className="usage-facility-bar-item">
                    ...
                    >
                      <span className="usage-facility-bar-name">{facility}</span>
