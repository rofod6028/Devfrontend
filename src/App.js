...
function UsageSummaryPage({ onBack }) {
  ...
  const [selectedIdx, setSelectedIdx] = useState(null); // 특정 달/년 클릭 시 상세 보기
  const [windowOffset, setWindowOffset] = useState(0); // 6개월 단위 구간 오프셋 (0 = 이번 달부터 6개월)
  const [expandedFacility, setExpandedFacility] = useState(null); // 전체기간 설비 랭킹에서 클릭해 펼친 설비명
  ...
  const WINDOW_SIZE = 6;
  // 이번 달을 항상 첫 구간(offset 0)의 시작점으로 삼는다
  const baseIdx = data.currentMonthIndex >= 0 ? data.currentMonthIndex : 0;
  const windowStart = baseIdx + windowOffset * WINDOW_SIZE;
  const monthlyWindow = data.monthly.slice(windowStart, windowStart + WINDOW_SIZE);

  // 6개월 구간을 몇 개까지 뒤로(미래로) 이동 가능한지 계산
  const maxOffset = Math.max(0, Math.ceil((data.monthly.length - baseIdx) / WINDOW_SIZE) - 1);
  const minOffset = 0; // 이번 달 이전(과거)은 요청상 표시하지 않음 — 항상 이번 달부터 시작
  ...
  // 선택된 구간(달/년)의 설비별/부품별 랭킹 상위 5개
  const selectedFacilityRanking = selected
    ? Object.entries(selected.byFacility).sort(([, a], [, b]) => b - a).slice(0, 5)
    : [];
  const selectedPartRanking = selected
    ? Object.entries(selected.byPart).sort(([, a], [, b]) => b - a).slice(0, 5)
    : [];

  // 전체기간 설비 랭킹에서 클릭해 펼친 설비의 부품 내역 (Top 8)
  // ✨ facilityPartBreakdown 자체가 없는 경우(예: 백엔드 배포 직전의 이전 응답)에도
  //    안전하게 빈 배열로 처리되도록 옵셔널 체이닝 적용 — 없으면 흰 화면 크래시 대신 "내역 없음" 표시
  ...
  return (
    <div className="usage-summary-page">
      ...
      <div className="usage-summary-print-area">
        {/* 상단 총계 카드 */}
        <div className="usage-summary-totals">
          <div className="usage-total-card">
            <span className="usage-total-label">전체 출고 건수</span>
            <span className="usage-total-value">{data.totalOutCount.toLocaleString()}건</span>
          </div>
          <div className="usage-total-card">
            <span className="usage-total-label">전체 출고 수량</span>
            <span className="usage-total-value">{data.totalOutQty.toLocaleString()}개</span>
          </div>
        </div>
        ...
        <div className="usage-period-tabs">
          <button
            ...
            onClick={() => { setPeriod('monthly'); setSelectedIdx(null); }}
          >
            월별
          </button>
          <button
            className={`usage-period-tab ${period === 'yearly' ? 'active' : ''}`}
            onClick={() => { setPeriod('yearly'); setSelectedIdx(null); }}
          >
            연도별
          </button>
        </div>
        ...
        {period === 'monthly' && (
          <div className="usage-window-selector">
            <button
              ...
              onClick={() => setWindowOffset(o => Math.max(minOffset, o - 1))}
            >
              ◀
            </button>
        ...
        <div className="usage-chart-card">
          ...
          <div className="usage-chart-bars">
            {entries.map((e, i) => {
              ...
              return (
                <div
                  key={e.key}
                  className={`usage-chart-bar-col ${selectedIdx === i ? 'selected' : ''}`}
          ...
          <p className="usage-chart-hint">막대를 클릭하면 해당 기간의 설비·부품 상세를 볼 수 있습니다.</p>
        </div>

        {/* 선택된 기간 상세 */}
        {selected && (
          <div className="usage-detail-card">
            <div className="usage-detail-title">{selected.label} 상세 — 총 {selected.total.toLocaleString()}개 출고</div>
            <div className="usage-detail-grid">
              <div className="usage-detail-col">
                <div className="usage-detail-col-title">🏭 설비별 사용량 (Top 5)</div>
                {selectedFacilityRanking.length === 0 && <div className="usage-empty">해당 기간 이력이 없습니다.</div>}
                {selectedFacilityRanking.map(([facility, qty], i) => (
                  <div key={facility} className="usage-rank-row">
                    <span className="usage-rank-num">{i + 1}</span>
                    <span className="usage-rank-name">{facility}</span>
                    <span className="usage-rank-qty">{qty}개</span>
                  </div>
                ))}
              </div>
              <div className="usage-detail-col">
                <div className="usage-detail-col-title">🔧 부품별 사용량 (Top 5)</div>
                {selectedPartRanking.length === 0 && <div className="usage-empty">해당 기간 이력이 없습니다.</div>}
                {selectedPartRanking.map(([model, qty], i) => (
                  <div key={model} className="usage-rank-row">
                    <span className="usage-rank-num">{i + 1}</span>
                    <span className="usage-rank-name">{model}</span>
                    <span className="usage-rank-qty">{qty}개</span>
                  </div>
                ))}
              </div>
        ...
        {/* 전체 기간 랭킹 — 어느 설비가 가장 많이 소모했는지 / 어떤 부품이 가장 많이 나갔는지 */}
        <div className="usage-detail-grid" style={{ marginTop: '14px' }}>
          <div className="usage-detail-card">
